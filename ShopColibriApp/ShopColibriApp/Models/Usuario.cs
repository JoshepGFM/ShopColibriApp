...
namespace ShopColibriApp.Models
{
    public class Usuario
    {
        ...
        public async Task<Usuario> GetUserData(string email)
        {
            try
            {
                string Route = string.Format("Usuarios/GetUsuario?email={0}", email);
        ...
        public async Task<List<Usuario>> GetUser()
        {
            try
            {
                string Route = string.Format("Usuarios/UsuariosPrincipal");
        ...
        public async Task<Usuario> GetUsuarioId(int id)
        {
            try
            {
                ...
                if (statusCode == HttpStatusCode.OK)
                {
                    var list = JsonConvert.DeserializeObject<Usuario>(response.Content);
                    return list;
                }
                else
                {
        ...
        public async Task<bool> ValidarLogin()
        {
            try
            {
                string Route = string.Format("Usuarios/ValidarLogin?Usuario={0}&pass={1}", this.Email, this.Contrasennia);
        ...
        public async Task<bool> ValidarConexion()
        {
            try
            {
                string Route = string.Format("Usuarios/ValidarConexion");
        ...
        public async Task<ObservableCollection<Usuario>> GetUsuarioBuscar(string? Filtro, bool? estado, bool cliente = true)
        {
            try
            {
                ...
                if (statusCode == HttpStatusCode.OK)
                {
                    var list = JsonConvert.DeserializeObject<ObservableCollection<Usuario>>(response.Content);
