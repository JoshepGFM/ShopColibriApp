...
namespace ShopColibriApp.Models
{
    class BitacoraSalida
    {
        ...
        public async Task<ObservableCollection<BitacoraSalida>> GetBitacoraSalida(DateTime inicio, DateTime final, string? producto, bool Todo)
        {
            try
            {
                ...
                string Route = string.Format("BitacoraSalidas/Consulta?inicio={0}&final={1}&producto={2}&Todo={3}", fechIni, fechFin, producto, Todo);
                ...
                {
                    var list = JsonConvert.DeserializeObject<ObservableCollection<BitacoraSalida>>(response.Content);
        ...
        public async Task<bool> PostBitacoraSalidas()
        {
            try
            {
                string Route = string.Format("BitacoraSalidas");
                string FinalURL = Servicios.CnnToShopColibri.UrlProduction + Route;
